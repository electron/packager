...
test('setting the quiet option does not print messages', (t) => {
  ...
  t.true(
    ...
    'quieted common.warning should not call console.warn',
  );
  info('info', true);
  t.true(
    console.error.notCalled,
    'quieted common.info should not call console.info',
  );
});
...
test('sanitize app name for use in file/directory names', (t) => {
  t.is(
    sanitizeAppName('@username/package'),
    '@username-package',
...
test('sanitize app name for use in the out directory name', (t) => {
  ...
  t.is(
    generateFinalBasename(opts),
    '@username-package-name-linux-x64',
    'generateFinalBasename output should be sanitized',
  );
});
...
test.serial(
  ...
  util.testSinglePlatform(async (t, opts) => {
    ...
    t.is(
      finalPath,
      path.join(t.context.workDir, generateFinalBasename(defaultOpts)),
    ...
    const resourcesPath = path.join(
      finalPath,
      util.generateResourcesPath(defaultOpts),
    );
    ...
    if (isPlatformMac(defaultOpts.platform)) {
      await util.assertDirectory(
        t,
        appPath,
        'The Helper.app should reflect opts.name',
      );
    } else {
      await util.assertFile(
        t,
        appPath,
        'The executable should reflect opts.name',
      );
    }
    await util.assertDirectory(
      t,
    ...
    await util.assertPathNotExists(
      ...
      'The output directory should NOT contain devDependencies by default (prune=true)',
    );
    await util.assertFilesEqual(
      t,
      path.join(opts.dir, 'main.js'),
      path.join(resourcesPath, 'app', 'main.js'),
      'File under packaged app directory should match source file',
    );
    await util.assertFilesEqual(
      t,
      path.join(opts.dir, 'ignore', 'this.txt'),
    ...
    await util.assertPathNotExists(
      t,
      path.join(resourcesPath, 'default_app'),
      'The output directory should not contain the Electron default_app directory',
    );
    await util.assertPathNotExists(
      t,
