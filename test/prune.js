...
test.serial(
  'prune: true',
  util.testSinglePlatform(async (t, baseOpts) => {
    await createPruneOptionTest(
      t,
      baseOpts,
      true,
      'package.json devDependency should NOT exist under app/node_modules',
    );
  }),
);

test.serial(
  'prune electron in dependencies',
  util.testSinglePlatform(async (t, baseOpts) => {
    const opts = {
      ...baseOpts,
...
test('isModule properly detects module folders', async (t) => {
  const isMod = (name) =>
    isModule(
      util.fixtureSubdir(path.join('prune-is-module', 'node_modules', name)),
    );
  const [mod, notMod, namespaced] = await Promise.all([
    isMod('module'),
    isMod('not-module'),
    isMod('@user/namespaced'),
  ]);
