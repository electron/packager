...
async function inferElectronVersionTest(t, opts, fixture, packageName) {
  ...
  return t.true(
    ...
    `The version should be inferred from installed ${packageName} version`,
  );
}
...
async function copyFixtureToTempDir(t, fixtureSubdir) {
  ...
  if (tmpdirPkg) {
    throw new Error(
      `Found package.json in parent of temp directory, which will interfere with test results. Please remove package.json at ${tmpdirPkg}`,
    );
  }
...
async function inferMissingVersionTest(t, opts) {
  ...
  t.is(
    ...
    'The version should be inferred from installed electron module version',
  );
}
...
async function testInferWin32metadataAuthorObject(
  t,
  opts,
  author,
  expected,
...
test(
  'infer using `electron-nightly` package',
  util.testSinglePlatform(
    inferElectronVersionTest,
    'infer-electron-nightly',
    'electron-nightly',
  ),
);
test(
  'infer using `electron` package only',
  util.testSinglePlatform(inferMissingVersionTest),
);
test(
  'infer win32metadata',
  util.testSinglePlatform(async (t, opts) => {
    ...
    return testInferWin32metadata(
      t,
      opts,
      expected,
      'win32metadata matches package.json values',
    );
  }),
);
test(
  'do not infer win32metadata if it already exists',
  util.testSinglePlatform(async (t, opts) => {
    ...
    return testInferWin32metadata(
      ...
      opts,
      expected,
      'win32metadata did not update with package.json values',
    );
  }),
);
test(
  'infer win32metadata when author is an object',
  util.testSinglePlatform(async (t, opts) => {
    const author = {
      name: 'Foo Bar Object',
    ...
    return testInferWin32metadataAuthorObject(
      t,
      opts,
      author,
      expected,
...
test(
  'do not infer win32metadata.CompanyName when author is an object without a name',
  util.testSinglePlatform(async (t, opts) => {
    const author = {
      email: 'foobar@example.com',
    };
    ...
    return testInferWin32metadataAuthorObject(
      ...
      author,
      expected,
      'win32metadata.CompanyName should not have been inferred',
    );
  }),
);
test(
  'infer: missing author for win32 target platform',
  util.testSinglePlatform(async (t, opts) => {
    opts.dir = await copyFixtureToTempDir(t, 'infer-win32metadata');
    opts.appVersion = '1.0.0';
    ...
    await fs.writeJson(packageJSONFilename, packageJSON);
    await t.throwsAsync(getMetadataFromPackageJSON(['win32'], opts, opts.dir), {
      message: /following fields: author/,
    });
  }),
);
test(
...
test(
  'missing Electron version from package.json',
  util.testSinglePlatform(
    inferFailureTest,
    'infer-missing-electron-version',
...
test(
  ...
  util.testSinglePlatform(
    inferFailureTest,
    'infer-missing-package-json',
    /^Could not locate a package\.json file/,
  ),
);
test(
  'infer with bad fields',
  util.testSinglePlatform(
    inferFailureTest,
    'infer-bad-fields',
