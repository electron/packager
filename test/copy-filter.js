...
const {
  ...
  userPathFilter,
} = require('../dist/copy-filter');
const fs = require('fs-extra');
const path = require('path');
const test = require('ava');
const util = require('./_util');

async function assertOutDirIgnored(
  t,
  opts,
  existingDirectoryPath,
  pathToIgnore,
  ignoredBasenameToCheck,
) {
  ...
  await util.assertPathNotExists(
    t,
    path.join(resourcesPath, 'app', ignoredBasenameToCheck),
    'Out dir must not exist in output app directory',
  );
}
...
async function assertFileIgnored(t, targetDir, ignoredFile) {
  await util.assertPathNotExists(
    t,
    path.join(targetDir, ignoredFile),
    `Ignored file '${ignoredFile}' should not exist in copied directory`,
  );
}
...
async function assertFileNotIgnored(t, targetDir, notIgnoredFile) {
  await util.assertPathExists(
    t,
    path.join(targetDir, notIgnoredFile),
    `The expected output directory should exist and contain ${notIgnoredFile}`,
  );
}
...
test(
  'ignore default: node_gyp_bins files',
  util.testSinglePlatform(ignoreTest, null, 'node_gyp_bins'),
);
test(
  'ignore default: .o files',
  util.testSinglePlatform(ignoreTest, null, 'ignore.o'),
);
test(
  'ignore default: .obj files',
  util.testSinglePlatform(ignoreTest, null, 'ignore.obj'),
);
test(
  'ignore default: package manager lock files',
...
test(
  'ignore: string in array',
  util.testSinglePlatform(ignoreTest, ['ignorethis'], 'ignorethis.txt'),
);
test(
  'ignore: string',
  util.testSinglePlatform(ignoreTest, 'ignorethis', 'ignorethis.txt'),
...
test(
  ...
  util.testSinglePlatform(
    ignoreTest,
    (file) => file.match(/ignorethis/),
    'ignorethis.txt',
  ),
);
test(
  'ignore: string with slash',
  util.testSinglePlatform(
    ignoreTest,
    'ignore/this',
...
test(
  'ignore: only match subfolder of app',
  util.testSinglePlatform(
    ignoreTest,
    'electron-packager',
...
test(
  'ignore: not junk when junk: false',
  util.testSinglePlatform(async (t, opts) => {
    opts.dir = util.fixtureSubdir('ignore-junk');
    opts.junk = false;
    const targetDir = await copyDirToTempDirWithIgnores(t, opts);
