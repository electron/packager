...
test.beforeEach(async (t) => {
  t.context.workDir = await fs.mkdtemp(
...
module.exports = {
  ...
  assertPathExistsCustom: async function assertPathExistsCustom(
    t,
    pathToCheck,
    expected,
    message,
  ...
  assertPathNotExists: async function assertPathNotExists(
    t,
    pathToCheck,
    message,
  ) {
  ...
  assertWarning: function assertWarning(t, message) {
    t.true(
      console.warn.calledWithExactly(message),
      `console.warn should be called with: ${message}`,
    );
  },
  ...
  invalidOptionTest: function invalidOptionTest(opts, err, message) {
    ...
    return (t) =>
      t.throwsAsync(
        packager({ ...packagerTestOptions(t), ...opts }),
        expectation,
        message,
      );
  },
  packageAndEnsureResourcesPath: async function packageAndEnsureResourcesPath(
    t,
    opts,
  ) {
    ...
    const resourcesPath = path.join(
      paths[0],
      module.exports.generateResourcesPath(opts),
    );
    await module.exports.assertDirectory(
      t,
      resourcesPath,
  ...
  parsePlist: async function parsePlist(t, appPath) {
    ...
    await module.exports.assertFile(
      t,
      plistPath,
      `The expected Info.plist should exist in ${path.basename(appPath)}`,
    );
  ...
  verifyPackageExistence: async function verifyPackageExistence(finalPaths) {
    return Promise.all(
      finalPaths.map(async (finalPath) => {
        try {
