...
test.serial(
  ...
  util.testSinglePlatform(async (t, opts) => {
    ...
    await util.assertPathNotExists(
      ...
      'The output directory should not contain the Electron default_app.asar file',
    );
  }),
);
...
async function assertUnpackedAsar(t, resourcesPath) {
  await util.assertDirectory(
    ...
    'app.asar.unpacked should exist under the resources subdirectory when opts.asar_unpack is set',
  );
  await util.assertDirectory(
    t,
    path.join(resourcesPath, 'app.asar.unpacked', 'dir_to_unpack'),
    'dir_to_unpack should exist under app.asar.unpacked subdirectory when opts.asar-unpack-dir is set dir_to_unpack',
  );
}
  ...
  util.testSinglePlatform(async (t, opts) => {
    ...
    await Promise.all([
      util.assertFile(
        ...
        'app.asar should exist under the resources subdirectory when opts.asar is true',
      ),
      util.assertPathNotExists(
        t,
        path.join(resourcesPath, 'app'),
        'app subdirectory should NOT exist when app.asar is built',
      ),
...
test.serial(
  ...
  util.testSinglePlatform(async (t, opts) => {
    ...
    for (const incompatibleOption of ['ignore', 'prune', 'derefSymlinks']) {
      util.assertWarning(
        t,
        `WARNING: prebuiltAsar and ${incompatibleOption} are incompatible, ignoring the ${incompatibleOption} option`,
      );
    }
    ...
    await util.assertFile(
      t,
      path.join(resourcesPath, 'app.asar'),
      'app.asar should exist under the resources subdirectory when opts.prebuiltAsar points to a prebuilt asar',
    );
    await util.assertFilesEqual(
      t,
      opts.prebuiltAsar,
      path.join(resourcesPath, 'app.asar'),
      'app.asar should equal the prebuilt asar',
    );
    await util.assertPathNotExists(
      t,
      path.join(resourcesPath, 'app'),
...
test.serial(
  'prebuiltAsar: fail when set to directory',
  failedPrebuiltAsarTest(
    { prebuiltAsar: util.fixtureSubdir('asar-prebuilt') },
    /must be an asar file/,
  ),
);
test.serial(
  'prebuiltAsar: fail when specifying afterCopy',
  incompatibleOptionWithPrebuiltAsarTest({ afterCopy: [] }),
);
test.serial(
  'prebuiltAsar: fail when specifying afterPrune',
  incompatibleOptionWithPrebuiltAsarTest({ afterPrune: [] }),
);
