...
if (!(process.env.CI && process.platform === 'win32')) {
  ...
  test.serial(
    'warn if building for mas and not signing',
    util.packagerTest(async (t, baseOpts) => {
  ...
  test.serial(
    'update Login Helper if it exists',
    util.packagerTest(async (t, baseOpts) => {
      ...
      await util.assertPathExists(
        t,
        helperPath,
        'renamed Login Helper app exists',
      );
      const plistData = await util.parsePlist(t, helperPath);
      t.is(
        plistData.CFBundleExecutable,
        helperName,
        'CFBundleExecutable is renamed Login Helper',
      );
      t.is(
        plistData.CFBundleName,
        helperName,
        'CFBundleName is renamed Login Helper',
      );
      t.is(plistData.CFBundleIdentifier, 'com.electron.mastest.loginhelper');
      await util.assertPathExists(
        t,
        path.join(contentsPath, 'MacOS', helperName),
        'renamed Login Helper executable exists',
      );
    }),
  );
}
