import { debug, warning } from './common.js';
    ...
    for (const platform of selectedPlatforms) {
      if (usingOfficialElectronPackages(opts)) {
        if (!validOfficialPlatformArch(platform, arch)) {
          warnIfAllNotSpecified(
            opts,
            `The platform/arch combination ${platform}/${arch} is not currently supported by Electron Packager`,
          );
          continue;
          ...
          if (buildVersion && !officialBuildExists(opts, buildVersion)) {
            warnIfAllNotSpecified(
              opts,
              `Official ${platform}/${arch} support only exists in Electron ${buildVersion}`,
            );
            continue;
          }
        }
...
function unsupportedListOption(
  name: keyof typeof supported,
  value: unknown,
  supportedValues: Set<string>,
) {
  return new Error(
    `Unsupported ${name}=${value} (${typeof value}); must be a string matching: ${Array.from(
...
export function allOfficialArchsForPlatformAndVersion(
  platform: SupportedPlatform,
